    ...
    if engine == "sql":
        ...
        try:
            return check_sql(
                ...
                sample_id=cfg.get("sample_id") if cfg else None,
                sample_columns=cfg.get("sample_columns") if cfg else None,
            )
        finally:
            if connector:
                connector.close()

    if engine == "local" and connector:
        if not table:
            raise ValueError("engine=local requires table=")
        partition_filter = cfg.get("filter") if cfg else None
        if partition_filter:
            query_str = f"SELECT * FROM {table} WHERE {partition_filter}"
        else:
