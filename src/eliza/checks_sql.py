...
_INLINE_CHECKS = {
    "not_null": lambda col, v, d: f"COUNT(CASE WHEN {col} IS NULL THEN 1 END)",
    "not_negative": lambda col, v, d: f"COUNT(CASE WHEN {col} < 0 THEN 1 END)",
    "between": lambda col, v, d: (
        f"COUNT(CASE WHEN {col} < {v.get('min', 'NULL')} OR {col} > {v.get('max', 'NULL')} THEN 1 END)"
        if "min" in v and "max" in v
        else f"COUNT(CASE WHEN {col} < {v['min']} THEN 1 END)"
        if "min" in v
        else f"COUNT(CASE WHEN {col} > {v['max']} THEN 1 END)"
    ),
    "in_set": lambda col, v, d: (
        f"COUNT(CASE WHEN {col} NOT IN ({','.join(_sql_quote(x) for x in v.get('values', []))}) THEN 1 END)"
    ),
    "regex": lambda col, v, d: f"COUNT(CASE WHEN {_regex_not_match(col, v.get('pattern', ''), d)} THEN 1 END)",
    "is_email": lambda col, v, d: f"COUNT(CASE WHEN {_regex_not_match(col, _EMAIL_PAT, d)} THEN 1 END)",
